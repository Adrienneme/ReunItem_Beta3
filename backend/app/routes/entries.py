...
async def get_specific_item_route(entry_id: str, _=Depends(UserModels.get_current_active_user)):
    return ItemModels.get_specific_item(entry_id)
...
async def update_item_route(
    ...
    if contact_number: updates["contact_number"] = contact_number  # fixed mapping
...
def generate_match_route(entry_id: str, current_user=Depends(UserModels.get_current_active_user)):
...
def cancel_claim_route(entry_id: str, _=Depends(UserModels.get_current_active_user)):
